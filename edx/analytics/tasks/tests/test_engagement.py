...
from edx.analytics.tasks.engagement import EngagementTask
...
@ddt
class EngagementTaskMapTest(InitializeOpaqueKeysMixin, MapperTestMixin, unittest.TestCase):
    ...
    def setUp(self):
        super(EngagementTaskMapTest, self).setUp()
...
@ddt
class EngagementTaskReducerTest(ReducerTestMixin, unittest.TestCase):
    """
    Tests to verify that engagement data is reduced properly
    """

    task_class = EngagementTask

    def setUp(self):
        super(EngagementTaskReducerTest, self).setUp()

        self.reduce_key = (self.COURSE_ID, 'test_user', self.DATE, 'problem', 'foobar', 'completed', '0')

    def test_replacement_of_count(self):
        inputs = [1, 1, 1, 1]
        self._check_output_complete_tuple(
            inputs,
            (('\t'.join(self.reduce_key[:-1]), 4),)
        )
